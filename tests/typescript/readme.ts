...
NginxConfFile.create(filename)
    .then((conf) => {
        ...
        console.log(
            "http.server.listen: " +
                conf.nginx.http?.[0].server?.[0].listen?.[0]._value
        );
        console.log(
            "http.server.location.root:" +
                conf.nginx.http?.[0].server?.[0].location?.[3].root?.[0]._value
        );
	...
	*/
        ...
        conf.nginx.http?.[0].server?.[1].location?.[0]._add(
            "root",
            "/var/www/example.com"
        );
	...
	*/
        ...
        conf.nginx.http?.[0].server?.[0].location?.[0]._addVerbatimBlock(
