...
import { NginxParser } from "./parser";
...
export type NginxConfItem = NginxConfItemApi &
    IndexableConfItem &
    NginxConfItemProps;
...
const createConfItem = (
    ...
    const newContext: NginxConfItemApi = {
        _remove: (name, index) => {
            ...
            if (!node) return newContext;

            if (node[index]) {
                const removed = node.splice(index, 1);
                if (removed.length) file.emit("removed", removed[0]);
            }
        ...
        _getString: function (depth: number) {
            ...
            if (item._isVerbatim) buffer += " {" + (item._value || "") + "}";
            else if (item._value) buffer += " " + item._value;
            ...
            if (item.__isBlock || properties.length) {
                ...
                if (!item._root) buffer += prefix + "}\n";
            } else if (!item._root) {
                if (!item._isVerbatim) buffer += ";";
    ...
    const defineProperty = <K extends keyof NginxConfItemProps>(
        name: K,
        value: NginxConfItemProps[K]
    ): void => {
    ...
    if (isRoot) defineProperty("_root", true);
...
export class NginxConfFile extends events.EventEmitter {
    ...
    public constructor(
        tree: NginxParseTreeNode,
        options: NginxConfOptions = {}
    ) {
        super();
        this.files = [];
        ...
        this.nginx = createConfItem(
            this,
            (this as unknown) as IndexableConfItem,
            {
                name: "nginx",
                children: null,
                comments: [],
    ...
    public static async create(
        file: string,
        options?: NginxConfOptions | null
    ): Promise<NginxConfFile> {
        let opts: NginxConfOptions;
        if (typeof options === "function") opts = {};
        else opts = options || {};
        const tree = await new NginxParser().parseFile(file, "utf8");
        if (!tree) throw new Error("tree could not be generated");
        return new NginxConfFile(tree, opts).live(file);
    }
    ...
    public static async createFromSource(
        ...
        const tree = await new NginxParser().parse(source);
        if (!tree) throw new Error("tree could not be generated");

        return new NginxConfFile(tree, opts);
    }
}
